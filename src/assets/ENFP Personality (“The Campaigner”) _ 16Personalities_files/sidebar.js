...
$j(document).ready(function() {
        ...
        if (!Stage2) {
            ...
            var jqxhr = $j.post('/users/profile/create', formData, function (data) {
                ...
                .fail(function (data) {
                    ...
                    if (data.status == 403) {
                        $j('.results-send-email-error').html('Results already sent - please reload the page.');
                    } else {
    ...
    function checkSticky() {
        ...
        var insightWidgetHeight = $j('.insight-widget').height();
        ...
        if ($j(document).scrollTop() > stickyThreshold && $j(window).height() > 0 && $j(window).width() > 1010) {
            ...
            if (newMargin + topWidgetHeight + insightWidgetHeight + 540 < stoppingPoint) {
    ...
    $j(document).on('click', '.sidebar .action-signup', function () {
        ...
        var jqxhr = $j.post('/users/profile/create', formData, function (data) {
            ...
            .fail(function (data) {
                ...
                if (data.status == 403) {
                    $j('.sidebar .alert').html('Results already sent - please reload the page.').show();
                } else {
    ...
    $j('.sidebar .profile-link').click(function () {
        $j(this).select();
    });
    ...
    $j('#compare-dialog input').click(function () {
        $j(this).select();
    });
