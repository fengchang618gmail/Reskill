...
$j(document).ready(function() {
    ...
    $j('#mobile-menu-switch').click(function () {
        ...
        if (!$j('#mobile-navigation').is(':visible')) {
            ...
            if ($j('.profile-menu-wrapper').length == 1) {
                var navOffset = $j('.navigation').height() + 22;
            } else {
                var navOffset = $j('.navigation').height() + 23;
            }
            ...
            if (data.code == 200) {
                ...
                if (data.notifications.length > 0) {
                    ...
                    $j.each(data.notifications, function (index, notification) {
                        ...
                        toAppend += '<div class="timestamp"><span class="fa fa-circle"></span> ' + notification.timestamp + '</div></div>';
                        toAppend += '<div class="col-xs-2 icon"><span class="far fa-angle-right"></span></div></a>';

                    });

                    toAppend += '</div>';
                    toAppend += '<div class="row alert-bottom"><a href="/users/notifications"><span class="fal fa-bell"></span> View all</a></div>';
                ...
                if (remainingNotifications == 0) {
                    ...
                    $j('.navbar-static-top .toggle-badge').hide();
            ...
            if (data.code == 200) {
                ...
                if (data.updates.length > 0) {
                    ...
                    $j.each(data.updates, function (index, update) {
                        ...
                        toAppend += '<div class="timestamp"><span class="fa fa-circle"></span> ' + update.timestamp + '</div></div>';
                        toAppend += '<div class="col-xs-2 icon"><span class="far fa-angle-right"></span></div></a>';

                    });

                    toAppend += '</div>';
                    toAppend += '<div class="row alert-bottom"><a href="/users/updates"><span class="fal fa-bell"></span> View all</a></div>';
    ...
    $j(document).on('click', '#profile-menu .profile-link, #mobile-navigation .profile-link', function () {
        $j(this).select();
    });
    ...
    $j(document).on('click', '#profile-menu-logout', function () {
        ...
        $j('#profile-menu-logout').prop('disabled', true);
